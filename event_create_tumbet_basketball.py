#!/usr/bin/env python3
"""
Tumbet Basketball Complete API Parser

This script fetches live and prematch basketball matches from Tumbet's API (SportWide) and extracts:
- Team names
- Match odds (2-way - Team 1 Win, Team 2 Win)
- Match links
...
# Configuration
SITE_NAME = 'Tumbet'
OUTPUT_FILE = 'tumbet-basketball-formatted.txt'
ERROR_LOG_FILE = 'tumbet-basketball-error.json'
...
def fetch_live_games() -> List[int]:
    """
    Fetch live basketball game IDs from Tumbet API.
    ...
    print(f"  Found {len(game_ids)} live basketball games")
    return game_ids


def fetch_prematch_all_games() -> List[int]:
    """
    Fetch ALL prematch basketball game IDs from Tumbet API using comprehensive getheader endpoint.
    ...
    # Navigate: OT -> Sports -> Basketball (ID=2) -> Regions -> Champs -> GameSmallItems
    ot_data = data['OT']
    sports = ot_data.get('Sports', {})
    basketball = sports.get('2', {})  # Basketball = Sport ID 2
    
    if not basketball:
        print("  No basketball data found")
        return []
    ...
    print(f"  Found {len(game_ids)} prematch basketball games (comprehensive coverage)")
    return game_ids
...
def extract_2way_odds(game_ev: Dict) -> tuple:
    """
    Extract 2-way odds from basketball game event data.
    
    Tumbet API structure for basketball:
    game["ev"]: {
        "103": {  # Market ID 103 = Money Line (2-way)
            "selection_id": {
                "pos": 697,  # Position: 697 = Team 1, 698 = Team 2
                "coef": 1.85  # Coefficient (odds)
            },
            ...
        }
    }
    ...
    Returns:
        tuple: (team1_odds, team2_odds) or (None, None)
    """
    team1_odds = None
    team2_odds = None
    ...
    # Market ID "103" is the Money Line market for basketball in Tumbet/SportWide API
    if '103' in game_ev:
        market_2way = game_ev['103']
        
        # Extract odds from selections
        for selection_id, selection_data in market_2way.items():
            ...
            if position == 697:  # Team 1 win
                team1_odds = coefficient
            elif position == 698:  # Team 2 win
...
def parse_game_details(data: Dict, game_type: str = 'prematch') -> List[Dict]:
    ...
    for game in games_data:
        try:
            ...
            # Extract 2-way odds from game ev (events)
            game_ev = game.get('ev', {})
            team1_odds, team2_odds = extract_2way_odds(game_ev)
            
            # Build match URL (direct link to iframe content)
            # Basketball URL format same as soccer, just different sport
...
def save_formatted_matches(matches: List[Dict], output_file: str = 'tumbet-basketball-formatted.txt'):
    """
...
def main():
    ...
    try:
        print(f"Fetching {SITE_NAME} live and prematch basketball matches...")
        print("=" * 60)
