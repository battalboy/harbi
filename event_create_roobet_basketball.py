...
This script fetches live and prematch basketball matches from Roobet's API (Betsby) and extracts:
- Team names
...
# Configuration
SITE_NAME = 'Roobet'
OUTPUT_FILE = 'roobet-basketball-formatted.txt'
ERROR_LOG_FILE = 'roobet-basketball-error.json'
...
HEADERS = {
    ...
    'Referer': 'https://roobet.com/sports/basketball-2'
}
    ...
    try:
        ...
        if 'categories' in data:
            for cat_id, cat_data in data['categories'].items():
                # Only include basketball (sport_id = '2')
                if cat_data.get('sport_id') == '2':
...
def extract_2way_odds(event: Dict) -> tuple:
    """
    Extract 2-way odds from basketball event data.
    
    Betsby API structure for basketball:
    markets: {
        "219": {  # Market ID 219 is 2-way Money Line
            "": {
                "5": {"k": "1.85"},  # Team 1 (Home) win
                "4": {"k": "2.10"}   # Team 2 (Away) win
            }
        }
    }
    ...
    # Market ID "219" is the 2-way Money Line market in Betsby API for basketball
    if '219' in markets:
        market_2way = markets['219']
...
def parse_matches(data: Dict, endpoint_type: str = 'live', categories: Dict = None, tournaments: Dict = None) -> List[Dict]:
    ...
    for event_id, event in events.items():
        try:
            ...
            # Extract 2-way odds
            team1_odds, team2_odds = extract_2way_odds(event)
            
            # Build match URL with full path
            # Roobet URL format: https://roobet.com/sports/basketball/{category_slug}/{tournament_slug}/{event_slug}-{event_id}
            slug = desc.get('slug', '')
            ...
            if slug and category_slug and tournament_slug:
                match_url = f"https://roobet.com/sports/basketball/{category_slug}/{tournament_slug}/{slug}-{event_id}"
            elif slug:
...
def save_formatted_matches(matches: List[Dict], output_file: str = 'roobet-basketball-formatted.txt'):
    """
...
def main():
    ...
    try:
        ...
        if prematch_data:
            ...
            print(f"   Found {len(categories)} basketball categories and {len(tournaments)} tournaments")
