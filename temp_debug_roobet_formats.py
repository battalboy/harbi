"""
EXPERIMENTAL: Roobet Timestamp Format Debug Script

This is a DEBUG VERSION of event_create_roobet.py
Purpose: Check the format of timestamp fields from Roobet/Betby API
...
def main():
    ...
    try:
        ...
        if prematch_data:
            ...
            events = prematch_data.get('events', {})
            if events:
                print("\n" + "=" * 70)
                print("DEBUG: TIMESTAMP FORMAT ANALYSIS")
                print("=" * 70)
                
                # Save first event to JSON
                first_event_id = list(events.keys())[0]
                sample = {'sample_event': events[first_event_id]}
                with open('debug_roobet_sample.json', 'w') as f:
                    json.dump(sample, f, indent=2)
                print(f"✅ Saved full sample to: debug_roobet_sample.json")
                
                # Analyze first 3 events
                for i, (event_id, event) in enumerate(list(events.items())[:3]):
                    desc = event.get('desc', {})
                    competitors = desc.get('competitors', [])
                    if len(competitors) >= 2:
                        team1 = competitors[0].get('name', 'N/A')
                        team2 = competitors[1].get('name', 'N/A')
                        ...
                        print(f"  'scheduled' type: {type(scheduled)}")
                        if start_time:
                            print(f"  'start_time' value: {start_time}")
                            print(f"  'start_time' type: {type(start_time)}")
                        
                        # Show all desc keys for first match
                        if i == 0:
                            print(f"\n  All desc keys: {list(desc.keys())}")
