#!/usr/bin/env python3
"""
Compare Oddswar and Roobet soccer events to find matches with team name variations.
"""
...
def load_existing_matches():
    """Load existing matches from roobet_matches.csv."""
    matches = {}
    try:
...
def main():
    ...
    print(f"Loaded {len(existing_matches)} existing matches from roobet_matches.csv")
    ...
    # Read Roobet events
    roobet_events = []
    with open('roobet-formatted.txt', 'r', encoding='utf-8') as f:
        for line in f:
            teams = parse_formatted_line(line.strip())
            if teams:
    ...
    print(f"Roobet events: {len(roobet_events)}")
