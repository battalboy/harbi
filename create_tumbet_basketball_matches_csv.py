#!/usr/bin/env python3
"""
Create tumbet_basketball_matches.csv with Oddswar basketball team names and their probable Tumbet matches.
...
def create_matches_csv():
    """
    Create tumbet_basketball_matches.csv with Oddswar and probable Tumbet matches.
    ...
    print("=" * 60)
    print("🏀 Creating Tumbet Basketball Matches CSV")
    print("=" * 60)
    
    print("\n📂 Loading team names...")
    oddswar_teams = load_team_names('oddswar_basketball_names.txt')
    tumbet_teams = load_team_names('tumbet_basketball_names.txt')
    
    print(f"   Oddswar basketball teams: {len(oddswar_teams)}")
    print(f"   Tumbet basketball teams: {len(tumbet_teams)}")
    
    # Load existing matches if CSV already exists
    existing_entries = {}  # Maps Oddswar -> {'Tumbet': name, 'Confidence': score}
    preserved_100_confidence = {}  # Tracks 100.0 entries to preserve
    already_used_tumbet = set()
    csv_exists = False
        ...
        with open('tumbet_basketball_matches.csv', 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                if tumbet:  # Track already-used Tumbet teams
                    already_used_tumbet.add(tumbet)
        
        csv_exists = True
        print(f"\n📄 Found existing tumbet_basketball_matches.csv")
        print(f"   Total existing entries: {len(existing_entries)}")
        print(f"   Entries with matches: {len([e for e in existing_entries.values() if e['Tumbet']])}")
        print(f"   🔒 100.0 confidence entries (will be preserved): {len(preserved_100_confidence)}")
        
    except FileNotFoundError:
        print(f"\n📄 No existing tumbet_basketball_matches.csv found - will create new file")
    ...
    # Track which Tumbet teams are available (not already used by 100.0 entries)
    ...
    for i, oddswar_team in enumerate(all_oddswar_teams_list, 1):
        ...
        # Check if this team is in current oddswar_basketball_names.txt
            ...
            if csv_exists and oddswar_team in existing_entries and existing_entries[oddswar_team]['Tumbet']:
                # Has existing match but not 100.0 confidence - we can re-match
                old_match = existing_entries[oddswar_team]['Tumbet']
    ...
    print(f"\n📝 Writing to tumbet_basketball_matches.csv...")
    
    with open('tumbet_basketball_matches.csv', 'w', newline='', encoding='utf-8') as f:
    ...
    print(f"\n📄 Output: tumbet_basketball_matches.csv")
    print(f"{'='*60}")
