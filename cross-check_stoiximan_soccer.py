#!/usr/bin/env python3
"""
Compare Oddswar and Stoiximan soccer events to find matches with team name variations.
"""
...
def load_existing_matches():
    """Load existing matches from stoiximan_matches.csv."""
    matches = {}
    try:
...
def main():
    ...
    print(f"Loaded {len(existing_matches)} existing matches from stoiximan_matches.csv")
    ...
    # Read Stoiximan events
    stoiximan_events = []
    with open('stoiximan-formatted.txt', 'r', encoding='utf-8') as f:
        for line in f:
            teams = parse_formatted_line(line.strip())
            if teams:
    ...
    print(f"Stoiximan events: {len(stoiximan_events)}")
